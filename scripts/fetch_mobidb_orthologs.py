#!/usr/bin/env python3
"""
fetch_mobidb_orthologs.py — fetch MobiDB para los ortholog_ids de la tabla orthologs.

Usa la misma mobidb_cache.db que fetch_mobidb.py. Solo fetchea IDs que no estén
ya en cache (ya sea porque vinieron de proteins o porque ya se descargaron antes).
"""
...
def main() -> None:
    ...
    all_ids = sorted(
        {r[0] for r in con_main.execute("SELECT DISTINCT ortholog_id FROM orthologs")}
    )
    cached = {r[0] for r in con_cache.execute("SELECT uniprot_id FROM responses")}
    errors_done = {r[0] for r in con_cache.execute("SELECT DISTINCT uniprot_id FROM fetch_errors")}

    pending = [uid for uid in all_ids if uid not in cached and uid not in errors_done]
    print(f"Ortholog IDs únicos: {len(all_ids)}")
    print(f"Ya en cache:         {len(cached & set(all_ids))}")
    print(f"Ya con error:        {len(errors_done & set(all_ids))}")
    ...
    print()
    print("=== Resultados fetch_mobidb_orthologs ===")
